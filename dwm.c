...
applysizehints(Client *c, int *x, int *y, int *w, int *h, int interact)
{
    ...
    *h = MAX(1, *h);
    if (interact) {
        ...
        // Edit 20250118: Adjust positioning logic to prevent unexpected jumps.
        if (*x + *w > m->wx + m->ww)
            *x = m->wx + m->ww - *w - 2 * c->bw;
        if (*x < m->wx)
            *x = m->wx;
        if (*y + *h > m->wy + m->wh)
            *y = m->wy + m->wh - *h - 2 * c->bw;
        if (*y < m->wy)
            *y = m->wy;
    }
    if (*h < bh)
...
/* 		*w = bh; */
/* 	if (resizehints || c->isfloating || !c->mon->lt[c->mon->sellt]->arrange) { */
/* 		/1* see last two sentences in ICCCM 4.1.2.3 *1/ */
/* 		baseismin = c->basew == c->minw && c->baseh == c->minh; */
...
movemouse(const Arg *arg)
{
    ...
    do {
        ...
        case MotionNotify:
            ...
            // Ограничение координат в пределах текущего монитора
            m = c->mon;
            nx = MAX(nx, m->wx);
            nx = MIN(nx, m->wx + m->ww - WIDTH(c));
            ny = MAX(ny, m->wy);
            ny = MIN(ny, m->wy + m->wh - HEIGHT(c));

            // Отладочные сообщения
            fprintf(stderr, "Window geometry: x=%d, y=%d, w=%d, h=%d\n", nx, ny, c->w, c->h);
